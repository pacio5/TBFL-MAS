...
class Personal(nn.Module):
    def __init__(self, num_classes=10):
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(in_channels=1, out_channels=8, kernel_size=5, stride=1,
                                             padding='same'))
        self.pool1 = nn.Sequential(nn.MaxPool2d(2))
        self.conv2 = nn.Sequential(nn.Conv2d(in_channels=8, out_channels=32, kernel_size=5, stride=1,
                                             padding='same'))
        self.pool2 = nn.Sequential(nn.MaxPool2d(2))
        self.fc1 = nn.Sequential(nn.Linear(in_features=1568, out_features=512), nn.ReLU())
        self.pl1 = nn.Sequential(nn.Linear(in_features=512, out_features=1024), nn.ReLU())
        self.pl2 = nn.Sequential(nn.Linear(in_features=1024, out_features=512), nn.ReLU())
        self.fc2 = nn.Sequential(nn.Linear(in_features=512, out_features=num_classes))
